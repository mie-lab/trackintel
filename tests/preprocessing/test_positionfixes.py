...
def geolife_pfs_stps_long(pfs_geolife_long):
    pfs, stps = pfs_geolife_long.as_positionfixes.generate_staypoints(
        method="sliding", dist_threshold=25, time_threshold=5 * 60
    )
    return pfs, stps
...
class TestGenerate_staypoints:
    ...
    def test_generate_staypoints_index_start(self):
        ...
        _, stps_sliding = pfs_ori.as_positionfixes.generate_staypoints(
            method="sliding", dist_threshold=25, time_threshold=300
        )
    ...
    def test_generate_staypoints_groupby_sliding(self):
        ...
        pfs_groupby, stps_groupby = pfs_ori.as_positionfixes.generate_staypoints(
            method="sliding", dist_threshold=25, time_threshold=300
        )
        # stps detection using for loop
        pfs_for, stps_for = _generate_staypoints_original(
            pfs_ori, method="sliding", dist_threshold=25, time_threshold=300
        )
...
class TestGenerate_triplegs:
    ...
    def test_tripleg_generation_stability(self, geolife_pfs_stps_long):
        ...
        pfs_case2, tpls_case2 = pfs.drop("staypoint_id", axis=1).as_positionfixes.generate_triplegs(
            stps, method="between_staypoints"
        )
    ...
    def test_generate_triplegs_index_start(self, geolife_pfs_stps_long):
        ...
        _, tpls_case2 = pfs.drop("staypoint_id", axis=1).as_positionfixes.generate_triplegs(
            stps, method="between_staypoints"
        )
...
def _generate_staypoints_original(
    positionfixes,
    method="sliding",
    dist_threshold=50,
    time_threshold=300,
        ...
        for user_id_this in ret_pfs["user_id"].unique():
            ...
            while i < num_pfs:
                ...
                while j < num_pfs:
                    ...
                    dist = dist_func(
                        pfs[i][name_geocol].x, pfs[i][name_geocol].y, pfs[j][name_geocol].x, pfs[j][name_geocol].y
                    )
...
def _assert_geodataframe_equal(gdf1, gdf2, **kwargs):
    ...
    pd.testing.assert_frame_equal(
        gdf1.drop(gdf1.geometry.name, axis=1), gdf2.drop(gdf1.geometry.name, axis=1), **kwargs
    )
