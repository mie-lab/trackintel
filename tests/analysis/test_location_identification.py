...
def example_freq():
    ...
    sps = gpd.GeoDataFrame(data=list_dict, geometry="geom", crs="EPSG:4326")
    sps.index.name = "id"
    assert "location_id" in sps.columns
    assert sps.as_staypoints
    return sps
...
class TestLocation_Identifier:
    ...
    def test_unkown_recipe(self, example_staypoints):
        """Test if ValueError is risen if recipe is unknown"""
        with pytest.raises(ValueError):
            location_identifier(example_staypoints, recipe="UNKNOWN", pre_filter=False)
    ...
    def test_pre_filter(self, example_freq, default_kwargs):
        ...
        example_freq.loc[f, "activity_label"] = freq_recipe(example_freq[f])["activity_label"]
