...
def temporal_tracking_quality(source, granularity="all"):
    ...
    -------
    DataFrame
        per-user tracking quality dataframe.
    """
    df = source.copy()
    ...
    if granularity == 'all':
        ...
        raise AttributeError("granularity unknown. We only support ['all', 'day', 'hour']. "
                                f"You passed {granularity}")
    return quality
...
def _split_overlaps(df, granularity='day'):
    """Split input df that have a duration of several days or hours.

    Parameters
    ----------
    df : GeoDataFrame
        The source dataframe to perform the split
    ...
    geopandas.GeoDataFrame
        The geopandas.dataframe object after the spliting
    """
    if granularity == 'day':
