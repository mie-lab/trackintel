...
def generate_staypoints(
    ...
    # we could use the same structure as generate_location function
        ...
        ret_pfs = ret_pfs.groupby("user_id").apply(_generate_staypoints_dbscan_user, name_geocol, epsilon, num_samples)
        ...
        for combined_id, group in grouped_df:
            ...
            if int(staypoint_id) != -1:
                ...
                staypoint[name_geocol] = Point(group[name_geocol].x.mean(), group[name_geocol].y.mean())
    ...
    **Attention**: This function requires either a column ``staypoint_id`` on the 
    ...
    if method == "between_staypoints":
        ...
        for user_id_this in ret_pfs["user_id"].unique():
            ...
            if case == 1:
                generated_triplegs.extend(
                    _triplegs_between_staypoints_case1(positionfixes_user_this, staypoints, user_id_this)
                )

            # Case 2: Staypoints exist but there is no user_id given
            elif case == 2:
                generated_triplegs.extend(
                    _triplegs_between_staypoints_case2(positionfixes_user_this, staypoints, user_id_this)
                )
        ...
        if len(generated_triplegs) == 0:
            ret_tpls = gpd.GeoDataFrame(columns=columns_triplegs, geometry="geom", crs=ret_pfs.crs)
        else:
...
def _generate_staypoints_sliding_user(
    df, name_geocol, elevation_flag, dist_threshold=50, time_threshold=300, dist_func=haversine_dist
):
    """
...
def _generate_staypoints_dbscan_user(pfs, name_geocol, epsilon=100, num_samples=1):
...
def _triplegs_between_staypoints_case1(positionfixes, staypoints, user_id_this):
    ...
    generated_triplegs_list = [
        __get_tripleg_record_from_psfs(pfs_tripleg, user_id_this, min_nb_of_points=2)
    ] + generated_triplegs_list
...
def _triplegs_between_staypoints_case2(positionfixes, staypoints, user_id_this):
    ...
    for stp1, stp2 in zip(list(stps), list(stps)[1:]):
        ...
        pfs_tripleg = positionfixes[
            (stp1["finished_at"] <= positionfixes["tracked_at"]) & (positionfixes["tracked_at"] <= stp2["started_at"])
        ]
        generated_triplegs_list.append(__get_tripleg_record_from_psfs(pfs_tripleg, user_id_this, min_nb_of_points=3))
    ...
    generated_triplegs_list = [
        __get_tripleg_record_from_psfs(pfs_first_tripleg, user_id_this, min_nb_of_points=2)
    ] + generated_triplegs_list
    ...
    for idx, pf in positionfixes.iterrows():
        ...
        if status == "tripleg_starts":
            ...
            curr_tripleg = {
                ...
                "finished_at": None,
                "geom": [
                    (prev_pf[name_geocol].x, prev_pf[name_geocol].y),
                ],
                "pfs_ids": [
                    prev_idx,
                ],
