...
def generate_staypoints(positionfixes,
    ...
    # ret_pfs['staypoint_id'] should be float (missing values)
...
def generate_triplegs(positionfixes, staypoints=None, method='between_staypoints', gap_threshold=1000*60):
    ...
    **Attention**: This function requires either a column ``staypoint_id`` on the 
    ...
    method: str
        ...
        between two staypoints. This method requires either a column
        ``staypoint_id`` on the positionfixes or passing staypoints as an input.

    gap_threshold: float
        The temporal threshold for a gap in the positionfixes. If the time
        difference between two positionfixes exceeds the threshold, a new
        tripleg is started.
    ...
    if method == 'between_staypoints':
        ...
        ret_tpls.index.rename(name='id', inplace=True)
        ret_tpls.index = ret_tpls.index.astype('int64')
        # # ret_pfs['tripleg_id'] should be float (missing values)
...
def _triplegs_between_staypoints_case3(positionfixes, user_id_this, gap_threshold):
    ...
    for idx, pf in positionfixes.iterrows():
        ...
        if first_iteration:
            ...
            if ~np.isnan(prev_pf['staypoint_id']):
                if np.isnan(pf['staypoint_id']):
