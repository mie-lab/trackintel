...
def generate_staypoints(positionfixes, method='sliding',
    ...
    if method == 'sliding':
        ...
        ret_pfs['staypoint_id'] = None  # this marks all that are not part of a SP
        ...
        stps2pfs_map = ret_spts[['id', 'pfs_id']].set_index('id').to_dict()['pfs_id']
...
def _generate_staypoints_sliding_user(df, 
    ...
    pfs = df.sort_values('tracked_at').to_dict('records')
    num_pfs = len(pfs)
